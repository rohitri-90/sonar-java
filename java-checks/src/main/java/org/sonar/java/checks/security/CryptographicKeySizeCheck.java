...
public class CryptographicKeySizeCheck extends IssuableSubscriptionVisitor {
  ...
  private class MethodVisitor extends BaseTreeVisitor {
    ...
    public void visitMethodInvocation(MethodInvocationTree tree) {
      if (tree.symbol().type() != null && isKeyGenerator(tree.symbol().type().fullyQualifiedName())) {
        ...
        } else if (isInitMethod(methodName) && geInstanceKeyGenSymbol != null && initializeIsCalledOnPreviouslyDefinedKeyGen(tree)) {
          Integer minKeySize = findMinimumKeySizeFromAlgorithm(algorithm);
          if (minKeySize != -1) {
            checkAlgorithmParameterToReport(tree.arguments().get(0), minKeySize)
    ...
    private boolean isKeyGenerator(String mitInvocator) {
      return ("java.security.KeyPairGenerator".equals(mitInvocator))
        || ("javax.crypto.KeyGenerator".equals(mitInvocator));
    }
